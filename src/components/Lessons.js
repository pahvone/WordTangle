...
const Lessons = (props) => {
    ...
    const [lessonButtons, setLessonButtons] = useState({
      ...
      const startLesson = (_index, _diff) => {
        redirect(`/LessonPage?lang=${props.langPath.lang}&diff=${_diff}&index=${_index}`);
      };
      ...
      const allComplete = (difficulty) => {
        ...
        if (difficulty === "intermediate") {
          for (
            ...
            if (props.userLangs[props.currentLang].lessonProg["beginner"][i] !== 100)
              allComplete = false;
          }
        } else if (difficulty === "advanced") {
          for (
            var i = 0;
            i < props.userLangs[props.currentLang].lessonProg["intermediate"].length;
            i++
          ) {
            if (props.userLangs[props.currentLang].lessonProg["intermediate"][i] !== 100)
              allComplete = false;
          }
        }
        return allComplete;
