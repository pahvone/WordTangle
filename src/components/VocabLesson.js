...
const VocabLesson = () => {
    ...
    const handleTextInput = (event, qWordSwitch) => {
        ...
        if(!correct) {
            ...
            let typoCompare = "";
            if(Array.isArray(compareTo))
                typoCompare = compareTo[0].split('');
            else typoCompare = compareTo.split('');

            let typoCount = 0;

            for (let i = 0, j = 0; i < input.length && j < typoCompare.length; i++, j++) {
                ...
                <span className="align-middle">
                    <div className="formDiv">
                        <form onSubmit={(e) => handleTextInput(e, qWordSwitch)}>
                            <p><input ref={textInputRef} className="form-control form-control-lg" type="text" id="char" required></input></p>
        ...
        if (!endQuiz()) {
            ...
            return (
                <div className='container-fluid '>
                    ...
                    <div className='row my-5 justify-content-center'>
                        <div className='col-md-4'> <button className="btn skip-button w-100 text-center" onClick={handleSwitchInputMode}>
