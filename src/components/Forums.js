...
const Forums = () => {
  ...
  [
    {id: 0, title: "Write Finnish only", description: "Communicate in Finnish only!", latestPost: Date.now(), route: "/Forums/view-forum/finnish-comms"},
    {id: 0, title: "Write Spanish only", description: "Communicate in Spanish only!", latestPost: Date.now(), route: "/Forums/view-forum/spanish-comms"},
  ]
  const community=
    [
  ...
  const newsPosts = [
    {id: 0, title: "Welcome to our new forum! :)", poster: "siteAdmin69", creationDate: Date.now(), latestComment: Date.now(), forumCategory: "News"},
    {id: 1, title: "Please submit feedback telepathically ONLY", poster: "annoyedCustomerSupportRep", creationDate: Date.now(), latestComment: Date.now(), forumCategory: "News"}
  ]
  const ForumPosts = [
    {id: 2,title: "Wow, new forums!", poster: "username", creationDate: Date.now(), latestComment: Date.now(), forumCategory: "Community"},
   {id: 3, title: "I love WordTangle!!", poster:"real user", creationDate: Date.now(), latestComment: Date.now(), forumCategory: "Community"}
  ]
      ...
      <div className="pagecontainer">
        <div className="responsive-container">
          ...
          <div className="forums">
              {
                news.map(function(forum, index) {
                  ...
                  return (
                    <div className="subforum" key={forum.id} style={{backgroundColor: index % 2 ? "#838530" : "#bdbf3d"}}>
                      <div className="header">
                        <div className='navlink'><NavLink to={forum.route}>{forum.title}</NavLink></div>
          ...
          <div className="language-help">
            <div className="forum-header-container" onClick={() => setLanguageIsOpen((x) => !x)}>
              <div className="forum-title-header">
                LANGUAGE HELP <span style={{fontSize:12, color: "gray"}}>[{languageIsOpen ? "collapse" : "expand"}]</span>
              </div>
              <div className="latest-post-header">
          ...
          <div className="practice-together">
            <div className="forum-header-container" onClick={() => setPracticeIsOpen((x) => !x)}>
              <div className="forum-title-header">
                PRACTICE TOGETHER <span style={{fontSize:12, color: "gray"}}>[{practiceIsOpen ? "collapse" : "expand"}]</span>
              </div>
              <div className="latest-post-header">
          ...
          <div className="community">
            <div className="forum-header-container" onClick={() => setCommunityIsOpen((x) => !x)}>
              <div className="forum-title-header">
                COMMUNITY <span style={{fontSize:12, color: "gray"}}>[{communityIsOpen ? "collapse" : "expand"}]</span>
              </div>
              <div className="latest-post-header">
                {communityIsOpen ? "Latest Post" : ""}
              </div>
            </div>  
          <animated.div style={{overflow: "hidden", ...communityStyles}}>
            <div className="forums">
              {
